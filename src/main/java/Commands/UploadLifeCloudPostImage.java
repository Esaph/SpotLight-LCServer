...
public class UploadLifeCloudPostImage extends EsaphCommandLC
{
    ...
    public UploadLifeCloudPostImage(LifeCloudServer lifeCloudServer, LifeCloudServer.RequestHandler requestHandler, LogUtilsEsaph logUtilsRequest)
    {
    ...
    public void run() throws Exception
    {
        ...
        try
        {
            ...
            if(this.uploadImage(fileTemp, fileLifeCloudUpload))
            {
                super.requestHandler().getConnectionToSql();
                prInsertNewLifeCloudUpload =
                        (PreparedStatement) super.requestHandler().getCurrentConnectionToSql().prepareStatement(UploadLifeCloudPostImage.QUERY_INSERT_NEW_LIFECLOUD_POST);
                ...
                prInsertNewLifeCloudUpload.setShort(4, CMTypes.FPIC);
                ...
                for(int counterHashtags = 0; counterHashtags < jsonArrayHashtags.length(); counterHashtags++)
                {
                    ...
                    PreparedStatement preparedAddHashtagsForPost = (PreparedStatement) super.requestHandler().getCurrentConnectionToSql()
                            .prepareStatement(UploadLifeCloudPostImage.queryInsertNewHashtags);
    ...
    private boolean uploadImage(File TEMP, File FILE_AUDIO) throws Exception
    {
        try
        {
            super.requestHandler().getWriter().println("1");
            super.logUtilsRequest().writeLog("LifeCloud handling image, reading length...");
            long maxLength = Long.parseLong(super.requestHandler().readDataCarefully(10));
            super.logUtilsRequest().writeLog("LifeCloud handling image, length: " + maxLength);
            super.requestHandler().getWriter().println("1");
            long readed = 0;
            ...
            super.logUtilsRequest().writeLog("LifeCloud post image wird hochgeladen.");
            ...
            if(this.isImageFile(TEMP)) //Überprüft datei. this.isImageFile(FILE_AUDIO)
            {
                super.logUtilsRequest().writeLog("LifeCloud image post WRITTEN.");
                FileUtils.copyFile(TEMP, FILE_AUDIO);
                return true;
            }
            else
            {
                if(FILE_AUDIO != null) //nur ein mal löschen. :) =) :D
                {
                    FILE_AUDIO.delete();
                }
            }
    ...
    private boolean isImageFile(File filepath)
    {
        try
        {
